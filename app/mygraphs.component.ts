...
export class MyGraphsComponent implements DoCheck, OnInit {
   ...
   private myGraph: GraphData[];         // the actual data
   ...
   // TODO: make it so that it redraws the graphs when selectedStats changes
    ngOnInit(){
        let d3: any = D3;
        this.myGraph = this.gService.getData();

        let numStats: number = this.selectedStats.length;
        ...
        let svg_width: number = 1600;
        let svg_height: number = 900;
        ...
        let margin_top: number = 20,
           ...
           // margin_bottom: number = 100,
            margin_left: number = 40+axisWidth*numStats;
        let margin2_top: number = 450,
            margin2_right: number = 20,
           // margin2_bottom: number = 30,
            margin2_left: number = 40+axisWidth*numStats;
        let width: number = +svg_width - margin_left - margin_right,
            //height: number = +svg_height/3 - margin_top - margin_bottom,
            //height2: number = +svg_height/3 - margin2_top - margin2_bottom;
            height: number = svg_height/2,
            height2: number = svg_height/16;

        let ratio: number = height2/(height);
        ...
        let zoom: any = d3.zoom()           // define a zoom
            ...
            .translateExtent([[0, 0], [width, height*2]])     // region that can be zoomed
            .extent([[0, 0], [width, height*2]])      // what's the difference from the one above?
        ...
        // higher order function to resize stats
        // javascript version
        /*
        function resize(fn) {
            return function(x){
        ...
        */
        ...
        let tempHeight2: number = height + margin_top*3;
        let context: any = svg.append("g")
            .attr("class", "context")
            .attr("transform", "translate(" + margin2_left + "," + tempHeight2 + ")");
        



        // set domain range for x axes (same for all graphs)
        x.domain(d3.extent(this.myGraph, stats[0]));
        x2.domain(x.domain());
        ...
        for(let i: number =1; i<=numStats; i++){
        ...
        focus.append("path")
            ...
            y[i-1].domain(d3.extent(this.myGraph, stats[i]));
        ...
        for(let i:number=1; i<=numStats; i++){
            ...
            context.append("path")
                .datum(this.myGraph)
                .attr("class", "gs"+i)
                .attr("transform", "translate( 0," + height2*(i-1) + ")")
                .attr("d", line2[i-1]);

            // draw X axis for small graph
            context.append("g")
                .attr("class", "axis axis--x"+i)
                .attr("transform", "translate(0," + height2*(i-1/2) + ")")
                .call(xAxis);
        }
